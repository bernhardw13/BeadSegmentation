...
function varargout = segmentBeads_v0_3(varargin)
% segmentbeads_v0_3 MATLAB code for segmentBeads_v0_3.fig
%      segmentbeads_v0_3, by itself, creates a new segmentbeads_v0_3 or raises the existing
%      singleton*.
%
%      H = segmentbeads_v0_3 returns the handle to a new segmentbeads_v0_3 or the handle to
%      the existing singleton*.
%
%      segmentbeads_v0_3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in segmentbeads_v0_3.M with the given input arguments.
%
%      segmentbeads_v0_3('Property','Value',...) creates a new
%      segmentbeads_v0_3 or raises the existing singleton*.  Starting from
%      the left, property value pairs are applied to the GUI before
%      segmentBeads_v0_3_OpeningFcn gets called.  An unrecognized property
%      name or invalid value makes property application stop.  All inputs
%      are passed to segmentBeads_v0_3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help segmentBeads_v0_3

% Last Modified by GUIDE v2.5 20-Aug-2018 14:22:08

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @segmentBeads_v0_3_OpeningFcn, ...
                   'gui_OutputFcn',  @segmentBeads_v0_3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before segmentBeads_v0_3 is made visible.
function segmentBeads_v0_3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to segmentBeads_v0_3 (see VARARGIN)
% Choose default command line output for segmentBeads_v0_3
...
% UIWAIT makes segmentBeads_v0_3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = segmentBeads_v0_3_OutputFcn(hObject, eventdata, handles) 
...
Tactio=Factio/(4*pi*(r*10^-6)^2);
xactio=mean(x(ind1));
yactio=mean(y(ind1));
zactio=mean(z(ind1));
ind2=find(theta<=pi/2);
Freactio=-A*(r*10^-6)^2*sum(I(ind2,:));
Treactio=Freactio/(4*pi*(r*10^-6)^2);
