...
f = msgbox(strcat('Segmenting bead',' ',num2str(j),' ','of',' ',num2str(size(handles.Beads,2))),'status');
    ...
    x_low=iBeads(2)-radius_cutout;
    x_high=iBeads(2)+radius_cutout;
    y_low=iBeads(1)-radius_cutout;
    y_high=iBeads(1)+radius_cutout;
    z_low=iBeads(3)-radius_cutout;
    z_high=iBeads(3)+radius_cutout;
    if all([x_low>0,x_high<=NX,y_low>0,y_high<=NY,z_low>0,z_high<=z_end])%check for beads not fully in image
        %get bead-sniplet
